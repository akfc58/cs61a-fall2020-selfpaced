...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        target_place = self.place # start place
        counter = 0 # count if out of range
        for _ in range(int(self.min_range)):
            if target_place is None: # where target is out of tunnel
                return None
            target_place = target_place.entrance
            print("DEBUG:target_place is ", target_place)   
        relative_range = self.max_range - self.min_range # 标准答案使用的是calss atti: distance
        while (target_place != beehive) and (counter <= relative_range):
            result = rANTdom_else_none(target_place.bees)
            if result:
                return result
            target_place = target_place.entrance
            counter += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if not self.place:
            amount += self.damage
        copy_of_bees = this_place.bees[:] 
        for i in range(len(this_place.bees)):
            Insect.reduce_armor(copy_of_bees[i],amount)
        this_place.bees = [x for x in copy_of_bees if x.place != None]
        # END Problem 5
...
# BEGIN Problem 7
# The WallAnt classpython3 ok -q 07
class WallAnt(Ant):
    '''WallAnt will do nothing each turn but it's super strong.'''
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.true_queen = QueenAnt.true_queen # the 1 is the true QUEEN!
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE **Ant*"
        if self.true_queen != 1: # check if the queen is the true queen.
            self.reduce_armor(self.armor) # if it is the impostor, kill itself once it takes action, without throwing anything.
        else:
            ScubaThrower.action(self, gamestate) # first, do the same as parent class do(throw leaves and something).
            start_place = self.place.exit # saves the first place behind the queen as a start_place.
            while start_place is not None:
                if start_place.ant and (not start_place.ant.have_doubled):
                    start_place.ant.damage = start_place.ant.damage * 2
                    start_place.ant.have_doubled = True
                else:
                    pass
                start_place = start_place.exit #doubles the damages of ants behind the queen that haven't been doubled.
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.true_queen == 1:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
    ...
    def move_to(self, place):
        ...
        print("DEBUG: the bee moved to:", place)

    def blocked(self):
        """Return True if this Bee cannot advance to the next Place."""
        # Special handling for NinjaAnt
        # BEGIN Problem Optional
        return (self.place.ant is not None) and (self.place.ant.blocks_path if self.place.ant else False)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        "*** YOUR CODE HERE ***"
        copy_of_bees = self.place.bees[:]
        if self.place.bees:
            for each_bee in copy_of_bees:
                Insect.reduce_armor(each_bee, self.damage)
                print("DEBUG:", each_bee.armor)
        # END Problem Optional 1
...
class ContainerAnt(Ant):
    ...
    def remove_from(self, place):
        ...
        if place.ant is self:
            ...
            print("DEBUG: spical remove activated")
            Insect.remove_from(self, place)
        else:
            # default to normal behavior
            print("DEBUG: normal remove activated")
            Ant.remove_from(self, place)
...
def make_slow(action, bee):
    ...
    def slow_action(self, gamestate):
        if gamestate % 2 == 0:
            action(gamestate)
    return lambda gamestate: slow_action(bee, gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    damage = 2 # initial damage set to 2
    ...
    def insects_in_front(self, beehive):
        ...
        distance = 0
        insects_in_front_dict = {}
        while start_place != beehive:
            if start_place.ant and start_place.ant != self: # 还没有单独考虑container ant
                insects_in_front_dict[start_place.ant] = distance
            if start_place.bees:
                for each in start_place.bees:
                    insects_in_front_dict[each] = distance
            distance += 1
            start_place = start_place.entrance
        print("DEBUG:", insects_in_front_dict)
        return insects_in_front_dict
        # END Problem Optional 5

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        self.damage = LaserAnt.damage - distance * 0.2 - self.insects_shot * 0.05
    ...
    def action(self, gamestate):
        ...
        for insect, distance in insects_and_distances.items():
            ...
            print("DEBUG:",insect, distance, self.damage)
