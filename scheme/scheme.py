...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol not in self.bindings and self.parent:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        "*** YOUR CODE HERE ***"
        p = self.env
        return p.make_child_frame(self.formals, args)
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_body = Pair(expressions.first.rest, expressions.rest)
        lambda_func = do_lambda_form(lambda_body, env)
...
def do_and_form(expressions, env):
    ...
    if expressions.rest is nil:
        ...
        return is_true_primitive(scheme_eval(expressions.first, env)) \
            and do_and_form(expressions.rest, env)
