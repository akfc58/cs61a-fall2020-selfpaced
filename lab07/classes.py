...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        deck_copy = player.deck.cards[:]
        for i in range(len(deck_copy)):
            player.deck.cards[i].attack += other_card.attack
            player.deck.cards[i].defense += other_card.defense
            # print("DEBUG:", player.deck.cards)
        new_oppent_deck_cards = []
        for i in range(len(opponent.deck.cards)):
            print("DEBUG:",opponent.deck.cards)
            if not((other_card.attack == opponent.deck.cards[i].attack) or \
            (other_card.defense == opponent.deck.cards[i].defense)):
                new_oppent_deck.append(opponent.deck.cards[i])
                print("DEBUG:", new_oppent_deck_cards)
        opponent.deck.cards = new_oppent_deck_cards
